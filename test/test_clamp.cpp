...
TEST(TestClamp, clamp_within)
{
    ...
    Eigen::Vector3d clamped = test_trifinger_build_workflows::clamp(vec, 0, 4);
...
{
    Eigen::Vector3d vec(1, 2, 3);
    Eigen::Vector3d clamped = test_trifinger_build_workflows::clamp(vec, 2, 4);
...
TEST(TestClamp, clamp_above)
{
    ...
    Eigen::Vector4d clamped = test_trifinger_build_workflows::clamp(vec, 1, 3);
...
{
    Eigen::Vector4d vec(1, 2, 3, 4);
    Eigen::Vector4d clamped = test_trifinger_build_workflows::clamp(vec, 2, 3);
